...
def configured_account_pool_file():
    ...
    if not raw_path:
        requested_paths = list(DEFAULT_ACCOUNT_POOL_FILES)
    else:
...
DEFAULT_AGENT_ROLE = normalize_agent_role(os.getenv("WAGENT_AGENT_ROLE", "runner"))
...
CONFIG = {
    ...
    "STOP_ON_TARGET": env_flag("WAGENT_STOP_ON_TARGET", True),
    ...
    "SCOUT_OBSERVATION_MEMORY_FILE": os.getenv(
        "WAGENT_SCOUT_OBSERVATION_MEMORY",
        "wagent_scanner_observation_memory.json",
    ),
...
class WagentBrain:
    ...
    def __init__(self):
        ...
        self.scout_observation_memory = {"rooms": {}}
        ...
        self._load_scout_observation_memory()
    ...
    def _room_success_recipe(self, room_sig, action):
        ...
        if isinstance(recipes, dict):
            ...
            if recipe:
                return recipe
        return self._normalize_action_recipe([], fallback_action=clean_action)
    ...
    def _set_room_success_recipe(self, room, action, recipe=None):
        ...
        if not clean_action:
            return
        room.setdefault("recipes", {})[clean_action] = self._normalize_action_recipe(recipe, fallback_action=clean_action)
    ...
    def _normalize_scout_observation_room_entry(self, room_sig, data):
        entry = {
            "observed_exits": [],
            "confirmed_walks": {},
            "scan_targets": [],
        ...
        for action in data.get("observed_exits", []):
            clean_action = re.sub(r'\s+', ' ', str(action).strip().lower())
            if not clean_action:
                continue
            if not (
                self._is_safe_game_command(clean_action)
                or self._is_persistable_navigation_action(clean_action)
            ):
                continue
            if clean_action not in entry["observed_exits"]:
                entry["observed_exits"].append(clean_action)
        ...
        if isinstance(raw_confirmed_walks, dict):
            for action, to_room in raw_confirmed_walks.items():
                ...
                if not (
                    ...
                    continue
                entry["confirmed_walks"][clean_action] = clean_target

        for target in data.get("scan_targets", []):
            clean_target = normalize_room_name(target)
            if clean_target and clean_target not in entry["scan_targets"]:
                entry["scan_targets"].append(clean_target)

        return entry

    def _normalize_scout_observation_memory(self, data):
        memory = {"rooms": {}}
        if not isinstance(data, dict):
            return memory
        ...
        for room_sig, room_data in raw_rooms.items():
            ...
            if not clean_room or self._is_noisy_room_key(clean_room):
                continue
            memory["rooms"][clean_room] = self._normalize_scout_observation_room_entry(clean_room, room_data)
        return memory

    def _load_scout_observation_memory(self):
        self.scout_observation_memory = {"rooms": {}}
        path = CONFIG.get("SCOUT_OBSERVATION_MEMORY_FILE", "")
        if not path:
            return
        try:
            self.scout_observation_memory = self._normalize_scout_observation_memory(_locked_json_load(path))
            room_count = len(self.scout_observation_memory.get("rooms", {}))
            logger.info(f"🧾 已加载扫描观察记忆: rooms={room_count}")
        except Exception as e:
            logger.warning(f"⚠️ 扫描观察记忆加载失败: {e}")

    def _runner_scout_frontier_actions(self, room_sig, visible_exits=None):
        clean_room = normalize_room_name(room_sig)
        room_entry = self.scout_observation_memory.get("rooms", {}).get(clean_room, {})
        if not isinstance(room_entry, dict):
            return []

        room_success = self.room_graph.get(clean_room, {}).get("success", {})
        known_success = set()
        if isinstance(room_success, dict):
            for action, to_room in room_success.items():
                if self._action_needs_confirmation(clean_room, action, to_room):
                    continue
                known_success.add(action)
        actions = []

        for action in room_entry.get("observed_exits", []):
        ...
        for action in room_entry.get("confirmed_walks", {}).keys():
            if visible_exits and action not in visible_exits:
                continue
            if action in known_success:
                continue
        ...
        if actions:
            logger.debug("Runner scout observation candidates room=%s actions=%s", clean_room, actions)
    ...
    def _priority_room_fast_action(self, room_sig, visible_exits=None):
        configured_actions = self._configured_room_actions("PRIORITY_ROOM_ACTIONS", room_sig)
        ...
        # Explicitly configured room actions are sometimes used to force a live
        # confirmation of a candidate route; let them override skip heuristics.
        for action in configured_actions:
            if action in self.blocked_commands:
                continue
            if not (self._is_safe_game_command(action) or self._is_persistable_navigation_action(action)):
    ...
    def _target_route_action(self, current_room, visible_exits=None, log_route_hit=True):
        ...
        if step in self.blocked_commands:
            return None
        if self._should_skip_room_action(current_room, step):
            return None
        if self._looks_like_repeat_loop(step) and not self._allows_failed_room_retry(current_room, step):
            return None
        if (
    ...
    def _role_candidate_priority(self, room_sig, visible_exits):
        ...
        if role == "scanner":
            ...
            for action in self._runner_scout_frontier_actions(room_sig, visible_exits):
                priority.append(action)
    ...
    def _visible_frontier_actions(self, room_sig, visible_exits):
        room = self.room_graph.get(room_sig, {})
        success = room.get("success", {}) if isinstance(room, dict) else {}
        known_success = set(success.keys()) if isinstance(success, dict) else set()
    ...
    def _action_supported_by_snapshot(self, action, snapshot):
        clean_action = re.sub(r'\s+', ' ', str(action).strip().lower())
        if not clean_action:
            return False
        visible_exits = set(self._extract_exits(snapshot))
        if clean_action in visible_exits:
            return True
        low = (snapshot or "").lower()
        if re.search(rf"\bgo\s+{re.escape(clean_action)}\b", low):
            return True
        if re.search(rf"\b{re.escape(clean_action)}\b", low) and clean_action in {"north", "south", "east", "west", "up", "down"}:
            return True
        return False

    def _runner_pending_navigation_action(self, room_sig, snapshot):
        failure_pivot = self._is_semantic_failure_feedback(snapshot)
        for cmd in list(self.pending_commands):
            if not self._is_known_navigation_action(cmd):
                continue
            if (
                self._should_skip_room_action(room_sig, cmd)
                and not self._action_supported_by_snapshot(cmd, snapshot)
                and not failure_pivot
            ):
                continue
            if self._looks_like_repeat_loop(cmd):
                continue
            if self._action_failed_in_snapshot(cmd, snapshot):
                continue
            return cmd
        return None

    def _runner_known_exit_action(self, room_sig, visible_exits=None, snapshot=""):
        candidates = self._usable_room_success_actions(room_sig, visible_exits)
        if not candidates:
            return None

        recent_rooms = self._search_state().get("recent_rooms", [])[-12:]
        room = self.room_graph.get(room_sig, {})
        success = room.get("success", {}) if isinstance(room, dict) else {}
        ...
        for action in candidates:
            if self._action_failed_in_snapshot(action, snapshot):
                continue
            to_room = str(success.get(action, "")).strip().lower()
            destination_success = self.room_graph.get(to_room, {}).get("success", {}) if to_room else {}
            destination_degree = len(destination_success) if isinstance(destination_success, dict) else 0
            revisit_penalty = recent_rooms.count(to_room) if to_room else 0
            score = (destination_degree, revisit_penalty, len(action.split()))
            if best_score is None or score < best_score:
                best_score = score
                best_action = action
        return best_action

    def _runner_fast_path_action(self, room_sig, visible_exits, snapshot=""):
        if self._agent_role() != "runner":
            return None, ""

        priority_action = self._priority_room_fast_action(room_sig, visible_exits)
        if priority_action and not self._action_failed_in_snapshot(priority_action, snapshot):
            return priority_action, f"configured priority room action for {room_sig}"

        blind_transit_action = self._blind_transit_action(room_sig, visible_exits)
        if blind_transit_action and not self._action_failed_in_snapshot(blind_transit_action, snapshot):
            return blind_transit_action, f"blind transit toward {self._target_room()}"

        target_action = self._target_route_action(room_sig, visible_exits)
        if target_action and not self._action_failed_in_snapshot(target_action, snapshot):
            return target_action, "runner target-route fast path"

        scout_actions = self._runner_scout_frontier_actions(room_sig, visible_exits)
        if scout_actions:
            scout_action = scout_actions[0]
            if not self._action_failed_in_snapshot(scout_action, snapshot):
                return scout_action, "runner scout-confirmation fast path"

        frontier_actions = self._visible_frontier_actions(room_sig, visible_exits)
        if frontier_actions:
            return frontier_actions[0], "runner frontier fast path"

        strategic = self._strategic_route_action(room_sig)
        if strategic and self._action_matches_current_state(strategic, visible_exits):
            if strategic not in self.blocked_commands and not self._should_skip_room_action(room_sig, strategic):
                if not self._action_failed_in_snapshot(strategic, snapshot):
                    return strategic, "runner strategic fast path"

        known_exit = self._runner_known_exit_action(room_sig, visible_exits, snapshot)
        if known_exit:
            return known_exit, "runner known-exit fast path"

        pending_nav = self._runner_pending_navigation_action(room_sig, snapshot)
        if pending_nav:
            return pending_nav, "runner pending-navigation fast path"
    ...
    def _role_prompt_sections(self, room_sig, visible_exits):
        ...
        if role == "scanner":
            objective = (
                "Reach the target room quickly using shared map memory, then maximize local room coverage: "
                "prioritize visible exits not yet validated in memory before reusing known shortcuts."
            )
        elif role == "runner":
            objective = (
    ...
    def _effective_failed_room_actions(self, room_sig):
        ...
        temp_actions = list(self.room_temp_failed_actions.get(clean_room, []))
        if not actions and not temp_actions:
            return []
        success_actions = self._successful_room_actions(clean_room)
        effective = [action for action in actions if action not in success_actions]
    ...
    def _drop_failed_actions_conflicting_with_success(self):
        ...
        for room_sig, actions in self.room_failed_actions.items():
            effective = self._effective_failed_room_actions(room_sig)
            ...
            if isinstance(raw_hops, dict):
                for from_room, hop in raw_hops.items():
                    ...
                    normalized_record["hops"][clean_from_room] = {
                        ...
                        "learned_from_role": normalize_agent_role(hop.get("learned_from_role", "runner")),
                    }
    ...
    def _is_recent_ping_pong_loop(self):
        ...
        moved = [
            ...
            if isinstance(transition, dict) and transition.get("success") and transition.get("moved")
        ]
        if len(moved) < 3:
            return False

        recent = moved[-3:]
        room_path = [normalize_room_name(recent[0].get("from_room", ""))]
        room_path.extend(normalize_room_name(transition.get("to_room", "")) for transition in recent)
        if any(not room for room in room_path):
            return False
        if len(set(room_path)) != 2:
            return False
        return room_path[0] == room_path[2] and room_path[1] == room_path[3]
    ...
    def _blind_transit_enabled(self):
        return CONFIG.get("BLIND_TRANSIT_ENABLED", True) and self._agent_role() == "runner" and bool(self._target_room())
    ...
    def _should_skip_room_action(self, room_sig, action):
        if self._is_observe_action(action):
            return False
        clean_room = str(room_sig).strip().lower()
        clean_action = re.sub(r'\s+', ' ', str(action).strip().lower())
        pending = self.pending_recipe_step
    ...
    def observe_room(self, snapshot):
        ...
        visible = set(self._extract_exits(snapshot))
        ...
        if visible:
            ...
            success = room.get("success", {})
            if isinstance(success, dict):
                stale_actions = [a for a in list(success.keys()) if a not in visible]
    ...
    def _build_think_prompt(
        ...
        if "rules" not in templates:
            templates["rules"] = (
                ...
                "18. If multiple exits are visible, alternate between known-good exit reuse and trying at least one unseen exit to discover new frontier rooms.\n"
                ...
                "25. If [Agent role] is scanner, first route to [Target room] when one is given, then prefer visible exits not yet validated in map memory before reusing known shortcuts.\n"
                "26. If [Agent role] is runner, use map-memory successful actions to cross known rooms quickly and reduce local experimentation until you reach the target room or a frontier room.\n"
                "27. Treat [Role routing hint] as part of the current assignment for this run."
            )
...
"""
    ...
    def _contextual_interaction_action(self, snapshot):
        ...
        if "splinter" in text and "light splinter" not in self.blocked_commands:
    ...
    def _startup_guide_action(self, snapshot, room_sig):
        ...
        priority_action = self._priority_room_fast_action(room_sig, exits)
        if priority_action:
            return priority_action

        target_room = self._target_room()
        target_tutorial_room = bool(
            target_room
            and (
                target_room == "intro"
                or target_room == "limbo"
                or "tutorial" in target_room
            )
        )
        ...
        elif rs == "intro" and target_tutorial_room:
            preferred = ["exit tutorial", "begin adventure", "begin", "old bridge", "exit", "start again"]
        elif "limbo" in rs or rs == "intro":
            preferred = ["tutorial", "begin adventure", "begin", "old bridge", "exit tutorial", "exit"]
        else:
    ...
    def think(self, snapshot, last_feedback, room_sig):
        ...
        runner_fast_action, runner_fast_reason = self._runner_fast_path_action(room_sig, visible_exits_now, snapshot)
        if runner_fast_action:
            return {
                "progress_score": 9.6,
                "thought": runner_fast_reason,
                "action": runner_fast_action,
                "new_commands": [],
                "model_used": "runner-fast-path"
            }

        if not CONFIG["PURE_MODEL_MODE"]:
            # 非 pure-model 才启用更激进的脚本策略；pure-model 仍保留关键场景恢复。
        ...
        target_route_action = self._target_route_action(room_sig, visible_exits_now)
        ...
        try:
            ...
            if clean_action in self.blocked_commands or self._looks_like_repeat_loop(clean_action):
                clean_action = self._fallback_action(room_sig)
    ...
    try:
        ...
        while True:
            ...
            if last_action_sent and last_room_sig:
                ...
                frontier_progress = is_new_room or new_exits_added > 0 or moved_room

                # 经验学习以探索推进为主，不再把同房间信息变化视作成功。
                was_success = exploration_success
                ...
                ping_pong_loop = brain._is_recent_ping_pong_loop()

                if frontier_progress and not ping_pong_loop:
                ...
                if ping_pong_loop:
                    logger.warning(
                        "🧱 Detected two-room ping-pong loop: %s -> %s",
                        last_room_sig,
                        transition_room_sig,
                    )
            ...
            if CONFIG.get("STOP_ON_TARGET", True) and target_room and current_room_sig == target_room:
